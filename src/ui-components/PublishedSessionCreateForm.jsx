...
import { createPublishedSession } from "../graphql/mutations";
...
export default function PublishedSessionCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    setSessionPublished(initialValues.SessionPublished);
  ...
  return (
    <Grid
        ...
        try {
          ...
          const modelFieldsToSave = {
            publishedSessionSessionPublishedId:
              modelFields?.SessionPublished?.id,
            Title: modelFields.Title,
            Description: modelFields.Description,
          };
          await client.graphql({
            query: createPublishedSession.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFieldsToSave,
              },
      ...
      {...getOverrideProps(overrides, "PublishedSessionCreateForm")}
      {...rest}
    >
      <ArrayField
        lengthLimit={1}
